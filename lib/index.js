...
module.exports = class ResidentAdvisor {
  ...
  addYoutubeToTracks(tracks) {
    var promises = _.map(tracks, (track) => {
      return this.getYoutubeFor(track).then((new_track) => {return new_track})
    });
    
    Q.all(promises).then((things) => { 
      console.log(things);
  ...
  getYoutubes(dj) {
    this.getChartedTracksFor(dj).then((charted_tracks) => {
      this.addYoutubeToTracks(charted_tracks);
  ...
  getChartedTracksFor(dj) {
    var dfd = Q.defer();
    var dj_name = dj.replace(/\s/g, '').toLowerCase();
    request(`http://www.residentadvisor.net/dj/${dj_name}/tracks?sort=mostcharted`, (err, resp, html) => {
